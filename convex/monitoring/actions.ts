  ...
  handler: async (ctx, args): Promise<{ inventory: InventoryResult; chunks: number }> => {
    ...
    for (let attempt = 0; attempt < 2 && !reviewed; attempt++) {
      ...
      const outcome = await completeStructured({
        request: {
          role, schemaName: role === 'MODEL_FAST' ? 'source_inventory_review_v1' : 'source_inventory_v1', jsonSchema: role === 'MODEL_FAST' ? inventoryReviewSchema : inventoryJsonSchema, reasoningEffort: 'high', maxCompletionTokens: role === 'MODEL_FAST' ? 4_000 : 12_000,
          messages: [
