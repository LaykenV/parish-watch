...
- Set meetingAt only when one contiguous source span states the meeting date and optionally the time. If the date and time appear in separate spans, do not join them. If only the date appears in one contiguous span, use 00:00:00 as the time. If even the date is not citable from one span, set meetingAt to null.
- For minutes, apply the same rule: set meetingAt only when one contiguous source span states both the meeting date and time. If the date and time appear in separate spans, set meetingAt to null. Never combine separate excerpts to support one timestamp.
