...
export const shareIssue = httpAction(async (ctx, request) => {
  ...
  if (matchesIssueEtag(request.headers.get('If-None-Match'), issue.revision)) return new Response(null, { status: 304, headers })
  return new Response(issueShareHtml({ title: issue.title, summary: issue.summary, bodyName: issue.bodyName, placeName: issue.placeName, mode: issue.mode, canonicalUrl, shareUrl }), { headers })
})
...
export function matchesIssueEtag(header: string | null, revision: string): boolean {
  // Convex hosting weakens ETags and adds -gzip to compressed representations.
  return (header ?? '').split(',').some(value => {
    const tag = value.trim().replace(/^W\//, '')
    return tag === '*' || tag === `"${revision}"` || tag === `"${revision}-gzip"`
  })
}
